...
if __name__ == '__main__':
    ...
    for layout_name in ('lgl', 'fr'):
        for k in (5, 10, 15, 20):
            # NOTE: only some look decent
            if (layout_name, k) != ('lgl', 5):
                continue

            print('{:}, k = {:}'.format(layout_name, k))
            #layout_name = 'fr'
            #k = 5
            k_slice = 6
            k_threshold = 0.2
            graph_layout_fn = '../../figures/fig2_graph_layout_{:}_k{:}_slice{:}_threshold{:.2f}.tsv'.format(
                    layout_name,
            ...
            if (not os.path.isfile(graph_layout_fn)) or args.regenerate_layout:
                ...
                #print('Louvain community algorithm')
                #import louvain
                #part = louvain.find_partition(G, louvain.ModularityVertexPartition)

                #print('Get community markers')
                #ds.samplesheet['community'] = -1
                #icol = ds.samplesheet.columns.tolist().index('community')
                #for ip, p in enumerate(part):
                #    ds.samplesheet.iloc[p, icol] = ip
                #dsc = ds.split(phenotypes=['community'])
                #markers = []
                #for ip, dsi in dsc.items():
                #    dso = ds.query_samples_by_metadata('community != @ip', local_dict=locals())
                #    expi = dsi.counts.mean(axis=1)
                #    expo = dso.counts.mean(axis=1)
                #    ind = expi > 100
                #    upreg = (expi.loc[ind] / expo.loc[ind]).nlargest(10)
                #    print(ip)
                #    print(upreg)
            ...
            graph_plot_fn = '../../figures/fig2_graph_layout_{:}_k{:}_slice{:}_threshold{:.2f}.png'.format(
                    layout_name,
                    k, k_slice, k_threshold)

            print('Plot number of cells for each cell type in various patients')
            # NOTE: NKT could be gamma-delta T cells too:
            # http://cytobase.montp.inserm.fr/ftp/Phenotyping%20NK%20cells%20and%20NKT%20cells.pdf
            ...
            #fig.savefig('../../fig2A-G.png')
            #fig.savefig('../../fig2A-G.svg')

    print('Report cell subtypes')
    vs_by_celltype = {}
    ...
    vs = pd.read_csv('../../figures/scatter_subtypes_Tcells.tsv', index_col=0, sep='\t')
    vs_by_celltype['T cell'] = vs

    if False:
        print('Plot all markers')
        fig, axs = plt.subplots(4, 10, figsize=(18, 9), sharex=True, sharey=True)
        axs = axs.ravel()
        for gene, ax in zip(dsTpl.featurenames, axs):
    ...
    if False:
        print('Print only some markers, repeat and pick best-looking tSNE')
        vss = []
        for irep in range(10):
            print('replicate n. {:}'.format(irep+1))
            dsTpl.counts.log(inplace=True)
            vs = dsTpl.dimensionality.tsne(n_dims=2, perplexity=70 + 2 * irep, random_seed=irep)
            vss.append(vs)
            dsTpl.counts.unlog(inplace=True)
            ...
            fig.savefig('/tmp/T_cells_tsne_rep_{:}.png'.format(irep+1))
            vs.to_csv('/tmp/T_cells_tsne_rep_{:}.tsv'.format(irep+1), sep='\t', index=True)

        plt.ion()
        plt.show()

    if True:
        print('Print only some markers from good tSNE run')
        vs = pd.read_csv('../../figures/scatter_subtypes_Tcells.tsv', index_col=0, sep='\t')
        genes_select = [None, 'CD4', 'CD8A', 'CXCR4', 'CCR7', 'IL7R', 'SELL', 'PRF1']
        fig, axs = plt.subplots(2, 4, figsize=(7, 4), sharex=True, sharey=True)
        axs = axs.ravel()
        ...
        fig.savefig('../../figures/scatter_subtypes_Tcells.png')
        fig.savefig('../../figures/scatter_subtypes_Tcells.svg')
    ...
    vs = pd.read_csv('../../figures/scatter_subtypes_NKcells.tsv', index_col=0, sep='\t')
    vs_by_celltype['NK cell'] = vs

    if False:
        print('Plot all markers')
        dsNKpl.counts.log(inplace=True)
        vs = dsNKpl.dimensionality.tsne(n_dims=2, perplexity=60)
        dsNKpl.counts.unlog(inplace=True)
        fig, axs = plt.subplots(3, 10, figsize=(18, 7), sharex=True, sharey=True)
        axs = axs.ravel()
        for gene, ax in zip(dsNKpl.featurenames, axs):
    ...
    if False:
        ...
        for irep in range(10):
            print('replicate n. {:}'.format(irep+1))
            dsNKpl.counts.log(inplace=True)
            vs = dsNKpl.dimensionality.tsne(n_dims=2, perplexity=70 + 2 * irep, random_seed=irep)
            vss.append(vs)
            dsNKpl.counts.unlog(inplace=True)
            ...
            fig.suptitle('NK cells')
            plt.tight_layout(rect=[0.025, 0.04, 1, 0.96])
            fig.savefig('/tmp/NK_cells_tsne_rep_{:}.png'.format(irep+1))
            vs.to_csv('/tmp/NK_cells_tsne_rep_{:}.tsv'.format(irep+1), sep='\t', index=True)
    ...
    if True:
        ...
        fig.savefig('../../figures/scatter_subtypes_NKcells.png')
        fig.savefig('../../figures/scatter_subtypes_NKcells.svg')
    ...
    vs = pd.read_csv('../../figures/scatter_subtypes_Bcells.tsv', index_col=0, sep='\t')
    vs_by_celltype['B cell'] = vs

    if False:
        print('Plot all markers')
        #dsBpl.counts.log(inplace=True)
        #vs = dsBpl.dimensionality.tsne(n_dims=2, perplexity=60)
        #dsBpl.counts.unlog(inplace=True)
        fig, axs = plt.subplots(4, 10, figsize=(18, 10), sharex=True, sharey=True)
        axs = axs.ravel()
        for gene, ax in zip(dsBpl.featurenames, axs):
    ...
    if False:
        ...
        for irep in range(30):
            print('replicate n. {:}'.format(irep+1))
            dsBpl.counts.log(inplace=True)
            vs = dsBpl.dimensionality.tsne(n_dims=2, perplexity=20 + 5 * irep, random_seed=irep)
            dsBpl.counts.unlog(inplace=True)
            vss.append(vs)
            ...
            fig.suptitle('B cells')
            plt.tight_layout(rect=[0.025, 0.04, 1, 0.96])
            fig.savefig('/tmp/B_cells_tsne_rep_{:}.png'.format(irep+1))
            vs.to_csv('/tmp/B_cells_tsne_rep_{:}.tsv'.format(irep+1), sep='\t', index=True)
    ...
    if True:
        ...
        fig.savefig('../../figures/scatter_subtypes_Bcells.png')
        fig.savefig('../../figures/scatter_subtypes_Bcells.svg')
    ...
    vs = pd.read_csv('../../figures/scatter_subtypes_monocytes.tsv', index_col=0, sep='\t')
    vs_by_celltype['monocyte'] = vs

    if False:
        print('Plot all markers')
        dsMpl.counts.log(inplace=True)
        vs = dsMpl.dimensionality.tsne(n_dims=2, perplexity=60)
        dsMpl.counts.unlog(inplace=True)
        fig, axs = plt.subplots(3, 10, figsize=(18, 7), sharex=True, sharey=True)
        axs = axs.ravel()
        for gene, ax in zip(dsMpl.featurenames, axs):
    ...
    if False:
        ...
        for irep in range(30):
            print('replicate n. {:}'.format(irep+1))
            dsMpl.counts.log(inplace=True)
            vs = dsMpl.dimensionality.tsne(n_dims=2, perplexity=20 + 5 * irep, random_seed=irep)
            dsMpl.counts.unlog(inplace=True)
            vss.append(vs)
            ...
            plt.tight_layout(rect=[0.025, 0.04, 1, 0.96])
            fig.savefig('/tmp/monocytes_tsne_rep_{:}.png'.format(irep+1))
            vs.to_csv('/tmp/monocytes_tsne_rep_{:}.tsv'.format(irep+1), sep='\t', index=True)
    ...
    if True:
        ...
        fig.savefig('../../figures/scatter_subtypes_monocytes.png')
        fig.savefig('../../figures/scatter_subtypes_monocytes.svg')
